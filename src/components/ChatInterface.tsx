...
import { Send, AlertCircle, Bot, User, Loader2 } from 'lucide-react';
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden" dir="auto">
      <header className="bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-semibold text-slate-800">Support Agent</h1>
            <p className="text-sm text-slate-500">We usually reply instantly</p>
          </div>
        </div>
      </header>
      ...
      <div className="bg-white border-t border-slate-200 p-4 sm:p-6 sticky bottom-0">
        ...
        >
          <input
            ...
            disabled={isLoading}
            className="w-full bg-slate-50 border border-slate-200 rounded-full pl-6 pr-14 py-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50 text-slate-800"
            dir="auto"
          />
          <button
            type="submit"
